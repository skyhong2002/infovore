...
export async function buildBackloggdCard(data: SourceSnapshot<BackloggdExtra>): Promise<string> {
  ...
  const tile = (g: MediaEntry, i: number, last: boolean) =>
    h(
      ...
      h('span', { style: { fontSize: titleFontSize(g.title), fontWeight: 700, color: C.text, marginTop: 6, lineHeight: 1.25, wordBreak: 'break-word', display: 'block', lineClamp: MAX_TITLE_LINES, fontFamily: textFont(g.title, 'Roboto') } }, g.title),
      h(
        'div',
