...
const styles = `
  ...
  *{box-sizing:border-box}html{scroll-behavior:smooth}body{background:radial-gradient(circle at 78% -15%,#172039 0,transparent 34rem),var(--bg);color:var(--text);font:15px/1.55 Inter,system-ui,sans-serif;margin:0;min-height:100vh}a{color:var(--blue)}a:focus-visible{border-radius:5px;outline:2px solid var(--blue);outline-offset:4px}
  ...
  .board-head{align-items:end;display:flex;gap:30px;justify-content:space-between;margin-bottom:28px}.board-head h1{font-size:clamp(34px,5vw,52px);letter-spacing:-.045em;line-height:1.02;margin:7px 0 10px}.board-head p{color:var(--muted);margin:0;max-width:640px}.board-status{color:var(--quiet);font-size:12px;text-align:right}.board-status strong{color:var(--text);display:block;font-size:13px}.board-stats{border-bottom:1px solid var(--line);border-top:1px solid var(--line);display:grid;grid-template-columns:repeat(4,1fr);margin-bottom:38px}.board-stat{padding:16px 18px}.board-stat:first-child{padding-left:0}.board-stat+.board-stat{border-left:1px solid var(--line)}.board-stat strong{display:block;font-size:23px;letter-spacing:-.03em}.board-stat span{color:var(--quiet);font-size:10px;letter-spacing:.08em;text-transform:uppercase}.priority-grid{display:grid;gap:14px;grid-template-columns:repeat(2,minmax(0,1fr));margin-bottom:48px}.priority-panel{background:var(--surface);border:1px solid var(--line);border-radius:18px;padding:20px}.priority-panel-head{align-items:baseline;display:flex;justify-content:space-between;margin-bottom:14px}.priority-panel-head h2{font-size:17px;margin:0}.priority-panel-head a,.priority-panel-head span{color:var(--quiet);font-size:11px;text-decoration:none}.priority-list{display:flex;flex-direction:column}.priority-item{align-items:center;border-top:1px solid var(--line);display:grid;gap:12px;grid-template-columns:54px minmax(0,1fr);padding:12px 0}.priority-item:first-child{border-top:0;padding-top:0}.priority-item:last-child{padding-bottom:0}.priority-item img,.priority-item-placeholder{background:#20242a;border-radius:7px;height:70px;object-fit:cover;width:54px}.priority-item h3{font-size:14px;line-height:1.3;margin:3px 0}.priority-item-meta{color:var(--muted);font-size:11px}.source-pulse{display:grid;gap:10px;grid-template-columns:repeat(3,minmax(0,1fr))}.source-pulse .entry{min-height:104px}.section-heading{align-items:end;display:flex;gap:20px;justify-content:space-between;margin:0 0 16px}.section-heading h2{font-size:20px;margin:0}.section-heading p,.section-heading span{color:var(--quiet);font-size:12px;margin:0}.content-section{margin-top:50px}
  ...
  @media(max-width:560px){.site-header{padding-top:16px}.site-main{padding-bottom:58px}.site-nav a{padding-inline:9px}.page-intro,.board-head{align-items:flex-start;flex-direction:column}.page-intro-aside,.board-status{text-align:left}.board-stats{grid-template-columns:repeat(2,1fr)}.board-stat{border-bottom:1px solid var(--line);padding-left:14px}.board-stat:nth-child(odd){border-left:0;padding-left:0}.board-stat:nth-child(n+3){border-bottom:0}.source-pulse{grid-template-columns:1fr}.activity-row{gap:13px;grid-template-columns:48px minmax(0,1fr)}.activity-row time{grid-column:1/-1}.activity-cover{height:66px;width:48px}.activity-main h2{font-size:16px}.grid,.archive-actions{grid-template-columns:1fr}.site-footer{grid-template-columns:repeat(2,1fr)}.footer-about{grid-column:1/-1}.platform-hero{align-items:start;grid-template-columns:58px minmax(0,1fr);padding:18px}.platform-avatar{border-radius:13px;height:58px;width:58px}.platform-actions{flex-wrap:wrap}}
`;
...
export function shell(title: string, body: string, active: PageKey): string {
  ...
  const description = "Sky's personal infoboard for everything watched, read, played, heard, and attended.";
...
function priorityItem(activity: Activity): string {
  const when = activityWhen(activity);
  const cover = activity.image
    ? `<img src="${html(activity.image)}" alt="" loading="lazy">`
    : '<span class="priority-item-placeholder" aria-hidden="true"></span>';
  const details = [when.date, when.time ? `${when.time} GMT+8` : '', activity.extra.venue, activity.status]
    .filter(Boolean).map(html).join(' · ');
  return `<article class="priority-item">${cover}<div><a class="source-label" href="/platforms/${html(activity.source)}">${html(sourceLabel(activity.source))}</a>
...
export function homePage(
  ...
  const priorities = `<section class="priority-grid"><div class="priority-panel"><div class="priority-panel-head"><h2>Up next</h2><a href="/now">All plans →</a></div>
    ...
    ${current.length ? `<div class="priority-list">${current.map(priorityItem).join('')}</div>` : '<div class="empty">Nothing is marked as currently in progress.</div>'}</div></section>`;
