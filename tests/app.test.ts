...
test('profile, now and Wrapped pages render from durable activities', async () => {
  ...
  assert.doesNotMatch(homeHtml, /highlights from/);
  assert.doesNotMatch(homeHtml, /class="board-stats"/);
  assert.doesNotMatch(homeHtml, /class="priority-grid"/);
  assert.doesNotMatch(homeHtml, /More recent highlights/);
  assert.doesNotMatch(homeHtml, /Balanced timeline/);
  ...
  assert.match(cardsHtml, /id="word-cloud-plain"/);
  const plainCard = await app.request('/card/word-cloud-plain.png');
