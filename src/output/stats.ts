...
export function statsPage(ownerName: string, summary: TimeSpentSummary): string {
  ...
  const method = `<section class="content-section"><div class="section-heading"><div><div class="eyebrow">Honest accounting</div><h2>How these numbers are made</h2></div></div>
    <div class="grid">
    <div class="card"><span class="pill">Measured</span><p class="muted">YouTube time comes from per-video watch sessions (extension-measured seconds, otherwise the gap to the next activity capped at the video length). stats.fm time comes from individual stream durations, with week/month/year/lifetime read from stats.fm's full history.</p></div>
    <div class="card"><span class="pill">Estimated ~</span><p class="muted">Simkl and Kitsu report no per-session durations, so their time is the growth of each platform's lifetime total between syncs. It accumulates from the day this tracking was deployed and is day-granular, so "last 24h" means today in Taipei.</p></div>
    <div class="card"><span class="pill">Not counted</span><p class="muted">Backloggd, Goodreads, and manual events carry no usable time data and are left out rather than guessed at.</p></div>
    </div></section>`;
