import { narrativeKeywords, canonicalKeyword } from './keywords.js';
...
function terms(value: string, phrases: boolean): Set<string> {
  const text = clean(value), segments = [...segmenter.segment(text)].filter((s) => s.isWordLike);
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    const pair = /[\p{Script=Han}]/u.test(word + next.segment) ? word + next.segment.toLowerCase() : word + ' ' + next.segment.toLowerCase();
