...
function exportPng(source, destination, width, height = width) {
  const data = readFileSync(root + source).toString('base64');
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}"><rect width="100%" height="100%" fill="white"/><image href="data:image/png;base64,${data}" width="${width}" height="${height}" preserveAspectRatio="xMidYMid meet"/></svg>`;
...
exportPng(icon, 'assets/brand/apple-touch-icon.png', 180);
